...
from .diary.router import router as diary
from .ai.router import router as ai
app=FastAPI(title="Nadha Shop Ledger",version="1.0.0")
app.add_middleware(CORSMiddleware,allow_origins=settings.cors_origin_list,allow_credentials=True,allow_methods=["*"],allow_headers=["*"])
for router in (auth,shops,transactions,customers,suppliers,closings,products,inventory,profit,health,recovery,lost_sales,insights,audit,diary,ai): app.include_router(router,prefix="/api")
@app.get("/health")

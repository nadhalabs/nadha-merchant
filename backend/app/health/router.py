...
from ..models import DayClosing,LedgerEntry,LedgerKind,Transaction,TransactionType
...
def cash_review(db,shop_id,start,end):
    ...
    for c in closings:
        current=daily_totals(db,shop_id,c.date);difference=c.actual_cash-current["expected_cash"]
        rows.append({"date":c.date,"expected_cash":current["expected_cash"],"actual_cash":c.actual_cash,"difference":difference,"status":"matched" if difference==0 else "review","message":"Cash matched recorded activity" if difference==0 else "Difference requires review","transaction_href":f"/history?date={c.date}"})
...
def health(period:str="today",day:date|None=None,shop_id=Depends(shop_access),db:Session=Depends(get_db)):
    start,end=bounds(period,day);start_dt=datetime.combine(start,time.min,tzinfo=timezone.utc);end_dt=datetime.combine(end,time.max,tzinfo=timezone.utc);txs=db.scalars(select(Transaction).where(Transaction.shop_id==shop_id,Transaction.occurred_at>=start_dt,Transaction.occurred_at<=end_dt)).all();entries=db.scalars(select(LedgerEntry).where(LedgerEntry.shop_id==shop_id,LedgerEntry.occurred_at>=start_dt,LedgerEntry.occurred_at<=end_dt)).all()
    sales=sum((x.amount for x in txs if x.type==TransactionType.sale),Z);received=sum(((x.cash_amount or Z)+(x.upi_amount or Z) if x.payment_method and x.payment_method.value=="mixed" else (x.amount if x.payment_method and x.payment_method.value in ("cash","upi") else Z) for x in txs if x.type==TransactionType.sale),Z)+sum((x.amount for x in entries if x.kind==LedgerKind.customer_payment),Z)
